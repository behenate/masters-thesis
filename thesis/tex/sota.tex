...
Bayesowskie filtrowanie spamu było badane już we wczesnych pracach nad
automatycznym filtrowaniem poczty. Androutsopoulos i współautorzy porównali
Naive Bayes z podejściem pamięciowym i pokazali, że automatycznie uczone filtry
mogą wyraźnie przewyższać ręcznie konstruowane reguły słów kluczowych
~\autocite{androutsopoulos-naive-bayes-spam}. Nowsze prace nadal porównują z nimi
nowe rozwiązania. Janez-Martino i współautorzy porównali między innymi
reprezentacje TF-IDF i bag-of-words z klasyfikatorami Naive Bayes,
drzewami decyzyjnymi oraz SVM w zadaniu klasyfikacji wiadomości spamowych
...
Nowsze prace sprawdzają także modele językowe bez dostrajania. Si i
współautorzy porównali klasyfikację wykonywaną przez ChatGPT z Naive Bayes,
SVM, regresją logistyczną, siecią feed-forward i BERT-em. Na dużym zbiorze
anglojęzycznym metody nadzorowane uzyskały lepsze wyniki, natomiast ChatGPT był
bardziej konkurencyjny dla mniejszego zbioru chińskojęzycznego
~\autocite{si-chatgpt-spam-2024}. Rojas-Galeano ocenił ChatGPT, GPT-4 i
Flan-T5 na korpusie SpamAssassin. Flan-T5 uzyskał F1 równe 90\% przy
klasyfikacji skróconej treści, a GPT-4 95\% po wcześniejszym streszczeniu
wiadomości. Autor zaznaczył jednocześnie ograniczenie badania do jednego
korpusu oraz koszt inferencji większych modeli
~\autocite{rojas-galeano-zero-shot-spam-2024}. Wyniki te pokazują, że sam
prompt może wystarczyć w części warunków, lecz nie zastępuje oceny na kilku
źródłach ani analizy kosztu działania.

Przywołane badania nie porównują kilku sposobów dostrojenia tego samego małego
modelu z otwartymi wagami. W tej pracy sprawdzono cztery sformułowania zadania
klasyfikacji dla Qwen3-0.6B, a uzyskane wyniki zestawiono ze zużyciem zasobów i
przepustowością prostszych klasyfikatorów.
...
\centering
\small
\resizebox{\textwidth}{!}{\input{tex/tables/sota_method_scope}}
